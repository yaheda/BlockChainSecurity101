...
contract ETBToken is Ownable {
  ...
  // /// tx.origin should not be used for owner
  // modifier onlyOwner() {
  //   require(tx.origin == owner, "Restricted Acces");
  //   _;
  // }

  /// anyone can set dex address
  ...
  // overflow
  // no check that account has that amount
  function burn(address account, uint256 amount) external onlyEtbDex {
    require(balances[account] - amount >= 0, "Not enough balance");
